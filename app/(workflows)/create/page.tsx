...
function WorkflowEditor() {
  ...
  const [rightOpen, setRightOpen] = useState(false); // Default properties closed until a node is clicked

  // Flow State - FIX: Explicitly typed as <Node> and <Edge> to prevent 'never[]' errors
  ...
  const onConnect = useCallback(
    (params: Connection | Edge) =>
      setEdges((eds) => {
        // Construct a fully valid Edge object to satisfy TypeScript
        const newEdge: Edge = {
          ...params,
  ...
  // Selection Logic - FIX: Explicitly typed the node parameter as Node
  const onNodeClick = useCallback((_: React.MouseEvent, node: Node) => {
    setSelectedNodeId(node.id);
    setRightOpen(true); // Automatically open properties panel
  }, []);
